...
int GetCert(wchar_t* exefilename, wchar_t* disallowed_subject)
{
    ...
    WCHAR szDisallowedSubject[MAX_PATH];
...
# ifdef UNICODE
        ...
        if (mbstowcs(szDisallowedSubject, disallowed_subject, MAX_PATH) == -1)
        {
            printf("Unable to convert to unicode.\n");
            err = 102;
            __leave;
        ...
        if (!fResult)
        {
            _tprintf(_T("%ws CryptQueryObject failed with %x\n"), szFileName, GetLastError());
            err = 2;
            __leave;
        }
...
BOOL PrintCertificateInfo(PCCERT_CONTEXT pCertContext, WCHAR dis_name[])
{
    ...
    __try
    {
        ...
        if (lstrcmpW(dis_name, szName) != 0)
        {
            _tprintf(_T("Not a match to target token\n"));
            fReturn = TRUE;
            __leave;
        }
        _tprintf(_T("target token matched\n"));
